...
EOS
  ...
  describe 'python_deprecated_python_version' do
    context 'returns Python version when `python` present' do
      it do
        ...
        expect(Facter.value(:python_deprecated_python_version)).to eq('2.7.9')
      end
    end
    ...
    context 'returns nil when `python` not present' do
      it do
        ...
        expect(Facter.value(:python_deprecated_python_version)).to eq(nil)
      end
    end
  end

  describe 'python_deprecated_python2_version' do
    context 'returns Python 2 version when `python` is present and Python 2' do
      it do
        Facter::Util::Resolution.stubs(:exec)
        Facter::Util::Resolution.expects(:which).with('python').returns(true)
        Facter::Util::Resolution.expects(:exec).with('python -V 2>&1').returns(python2_version_output)
        expect(Facter.value(:python_deprecated_python2_version)).to eq('2.7.9')
      end
    end
    ...
    context 'returns nil when `python` is Python 3 and `python2` is absent' do
      it do
        ...
        expect(Facter.value(:python_deprecated_python2_version)).to eq(nil)
      end
    end
  ...
  describe 'python_deprecated_python3_version' do
    context 'returns Python 3 version when `python3` present' do
      it do
        ...
        expect(Facter.value(:python_deprecated_python3_version)).to eq('3.3.0')
      end
    end
    ...
    context 'returns nil when `python3` not present' do
      it do
        ...
        expect(Facter.value(:python_deprecated_python3_version)).to eq(nil)
      end
    end
  end
end
